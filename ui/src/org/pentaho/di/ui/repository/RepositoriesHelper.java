      ...
      @Override public void invoke( Map<String, Object> map ) {
        ...
        try {
          RepositoryMeta
              repositoryMeta =
              PluginRegistry.getInstance().loadClass( RepositoryPluginType.class, id, RepositoryMeta.class );
          repositoryMeta.build( map );

          input.addRepository( repositoryMeta );
          fillRepositories();
          model.setSelectedRepository( repositoryMeta );
          writeData();
        } catch ( Exception e ) {
          log.logDetailed(
              BaseMessages.getString( PKG, "RepositoryLogin.ErrorCreatingRepository", e.getLocalizedMessage() ) );
          new ErrorDialog( shell, BaseMessages.getString( PKG, "Dialog.Error" ),
