...
public class SpoonTreeDelegate extends SpoonDelegate {
  ...
  public TreeSelection[] getTreeObjects( final Tree tree, Tree selectionTree, Tree coreObjectsTree ) {
    ...
    if ( selectionTree != null && !selectionTree.isDisposed() && tree.equals( selectionTree ) ) {
      return getSelectionTreeObjects( selectionTree );
    }
    if ( tree != null && coreObjectsTree != null && tree.equals( coreObjectsTree ) ) {
      return getCoreObjectsTreeObjects( coreObjectsTree );
    }
    // doc says null but this was prev behavior (and tree==null && selectionTree != null -> NPE)
    return new TreeSelection[0];
    //return objects.toArray( new TreeSelection[objects.size()] );
  }

  private TreeSelection[] getCoreObjectsTreeObjects( Tree coreObjectsTree ) {
    List<TreeSelection> objects = new ArrayList<TreeSelection>();
  ...
  private TreeSelection[] getSelectionTreeObjects( Tree selectionTree ) {
    List<TreeSelection> objects = new ArrayList<TreeSelection>();
      ...
      switch ( path.length ) {
        ...
        case 3: // ------complete-----
          if ( path[0].equals( Spoon.STRING_TRANSFORMATIONS ) ) { // Transformations title
            ...
            // TODO: as in other calls, objects has all previous selections (if any) and not the current one...
  ...
  // TODO: calls to extension point have all selections but the current one...
  // TODO: is there ever any selection? <-- nope, we're limiting selections to 1 
  private void executeExtensionPoint( SpoonTreeDelegateExtension extension ) {
//    //TODO printf
//    System.out.println( "executeExtensionPoint: #objects=" + extension.getObjects().size() );
    try {
